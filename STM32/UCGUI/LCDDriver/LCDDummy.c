...
unsigned int LCD_L0_GetPixelIndex(int x, int y) {
  LCD_PIXELINDEX PixelIndex = LCD_ReadPoint(y, 479 - x);
  return PixelIndex;
}
...
/*	unsigned int i = x1 - x0 + 1;
	AddressSet(x0, y, x1, y);
	while(i--) {
...
/*	unsigned int i = y1 - y0 + 1;
	AddressSet(x, y0, x, y1);
	while(i--) {
		LCD_RAM(LCD_COLORINDEX);
	}*/
  if (GUI_Context.DrawMode & LCD_DRAWMODE_XOR) {
...
void LCD_L0_FillRect(int x0, int y0, int x1, int y1) {
	unsigned int i = (x1 - x0 + 1) * (y1 - y0 + 1);
	AddressSet(y0, 479 - x1, y1, 479 - x0);
	while(i--) {
		LCD_RAM(LCD_COLORINDEX);
	}
}
